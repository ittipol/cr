class Images {
	...
	init(){
		let token = new Token();
		this.code = token.generateToken(40);

  	let hidden = document.createElement('input');
    hidden.setAttribute('type','hidden');
    hidden.setAttribute('name','Image[token]');
	...
	createAddedImage(parent,code,index,filename,description='') {
	  ...
	  let hiddenInputName = 'Image[images]['+index+'][filename]';
	  if((this.type == 'profile-image') || (this.type == 'cover')) {
	  	hiddenInputName = 'Image[image][filename]';
	  }
	  ...
	  if(this.style == 'description'){
	  	document.getElementById(code+'_textarea_'+index).setAttribute('name','Image[images]['+index+'][description]');
	  }
	...
	createDeletedImage(parent,index,value) {
		let hidden = document.createElement('input');
	  hidden.setAttribute('type','hidden');
	  hidden.setAttribute('name','Image[delete]['+index+']');
	...
	_createUploader(index,image){
		...
		let html = '';
		html += '<div class="image-panel '+this.style+' clearfix">';
		html += '<input type="hidden" name="Image[images]['+index+'][id]" value="'+image.id+'" >';
		html += '<label id="'+this.code+'_'+this.runningNumber+'" class="image-label">';
		html += '<input id="'+this.code+'_image_'+this.runningNumber+'" class="image-input" type="file">';
		...
		html += '</label>';
		if(this.style == 'description'){
			html += '<textarea id="'+this.code+'_textarea_'+this.runningNumber+'" name="Image[images]['+index+'][description]" placeholder="คำอธิบายเี่ยวกับรูปภาพนี้">'+image.description+'</textarea>';
		}
		html += '</div>';
